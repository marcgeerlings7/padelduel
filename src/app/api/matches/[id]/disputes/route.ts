...
import { openMatchScoreDispute, DisputeError } from "@/server/services/disputeService";
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const dispute = await openMatchScoreDispute(params.id, user.id, parsed.data.reason);
